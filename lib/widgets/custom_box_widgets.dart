...
class UtilizationWidgets extends StatefulWidget {
  ...
  Color borderColor;
  bool networkWidget = false;
  final VoidCallback onTap;
  UtilizationWidgets({super.key,required this.title,required this.subTitle,required this.usages,this.iconColor,this.subTitle1,this.usages1,required this.borderColor,required this.onTap,required this.networkWidget});
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              widget.subTitle1==null ? const SizedBox(height: 20,) : const SizedBox(height: 5,),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(widget.subTitle,style: const TextStyle(fontSize: 15,color: Colors.white),),
                  Flexible(child: Text("${widget.usages.ceil().toString()}${widget.networkWidget!?"KiB/s":"%"}"??"0",style: TextStyle(color: widget.networkWidget ? Colors.greenAccent : widget.usages < 70 ? Colors.greenAccent : Colors.redAccent ,overflow: TextOverflow.ellipsis),))
                ],
              ),
              if(widget.subTitle1==null) const SizedBox(height: 10,),
              if(widget.subTitle1!=null) Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(widget.subTitle1!,style: const TextStyle(fontSize: 15,color: Colors.white),),
                  Flexible(child: Text("${widget.usages1!.ceil().toString()}${widget.networkWidget!?"KiB/s":"%"}"??"0",style: TextStyle(color: widget.networkWidget ? Colors.greenAccent :widget.usages < 70 ? Colors.greenAccent : Colors.redAccent ,overflow: TextOverflow.ellipsis),))
                ],
              ),
...
//                   shrinkWrap: true,
//                   itemCount: scanning.fullCpuUsageList.length,itemBuilder: (context, index) => Container(
//                     width: double.infinity,
//                     child: SelectableText(scanning.fullCpuUsageList[index],style: TextStyle(color: AppColors.primaryLightColor.withOpacity(0.5)),)),),
//               )),
